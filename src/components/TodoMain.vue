<template>
    <div class="TodoMain">
        ...
        <form @keyup.esc="(taskInput = ''), readTask()">
            <input
                class="default-input"
            ...
            >
                <div class="task">
                    ...
                    <div class="task-controls">
                        ...
                        <button v-else @click.prevent="toggleEdit(task.text)">
                            Edit
                        </button>
...
const taskList = [
    { id: 1, text: "Estudar Vue o domingo inteiro", done: false },
    { id: 2, text: "Lavar o carro", done: false },
    { id: 3, text: "Ler um livro", done: false },
];
...
const createTask = () => {
    ...
    for (let i = 0; i < taskList.length; i++) {
        if (taskList[i].id > maiorId) {
            maiorId = taskList[i].id;
        }
    }

    filteredTaskList.value.push({
        id: maiorId + 1,
    ...
    sortBy(
        ...
        "text",
        (result) => (filteredTaskList.value = result)
    );
    taskInput.value = "";
};
...
const readTask = () => {
    taskInput.value
        ? (filteredTaskList.value = taskList.filter((task) =>
              task.text.toLowerCase().includes(taskInput.value.toLowerCase())
          ))
